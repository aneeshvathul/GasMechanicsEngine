...
void ClosedSystem::createCircleBody(float x, float y){
        ...
        while (stepTimeAccumulator >= timeStep) {
            ...
            if (circleBodies.size() > 1){
                while (!collisionQueue.empty() && frameCount >= collisionQueue.front().getFrame()){
                    int first = collisionQueue.front().getFirst();
                    int second = collisionQueue.front().getSecond();
                    CircleBody& circleBody1 = circleBodies[first];
                    CircleBody& circleBody2 = circleBodies[second];
        ...
        for (int i = 0; i < circleBodies.size() - 1; i++){
            for (int j = i + 1; j < circleBodies.size(); j++){
                ...
                if (circleBody1.isCollidingWith(circleBody2)){
                    ...
                    collisionQueue.push(CollisionState(frameCount, i, j)); 
                }
                else{
                    ...
                    if (discriminant >= 0.f){
                        ...
                        if (t1 > 0.f && f1 > 0){
                            if (f1 < minFrame){
                                ...
                                collisionQueue.push(CollisionState(f1, i, j));
                                minFrame = f1;
                            }
                            else if (f1 == minFrame){
                                collisionQueue.push(CollisionState(f1, i, j));
                            ...
                            if (f2 < minFrame){
                                collisionQueue = std::queue<CollisionState>();
                                collisionQueue.push(CollisionState(f2, i, j));
                                minFrame = f2;
                            }
                            else if (f2 == minFrame){
                                collisionQueue.push(CollisionState(f2, i, j));
